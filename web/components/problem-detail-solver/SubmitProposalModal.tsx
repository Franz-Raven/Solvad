...
export default function SubmitProposalModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitProposal(problemId, proposedApproach, parentAttemptId, files);
      onSuccess();
    } catch (err: any) {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl border border-gray-200 flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">The Seeker will review your approach and attachments before generating a workspace.</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <form id="proposal-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="I will approach this by initially mapping out the database schema, then constructing the REST architecture..."
            ...
            <div>
              ...
              {/* Selected Files List with Local Preview Links */}
              {files.length > 0 && (
                  ...
                  {files.map((file, idx) => (
                    <li key={idx} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg text-sm shadow-sm">
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        {/* Clickable Local File Preview */}
                        <a 
                          href={URL.createObjectURL(file)} 
                          target="_blank" 
                          rel="noopener noreferrer"
                          title="Click to preview file"
                          className="truncate font-medium text-secondary hover:text-accent hover:underline transition-colors"
                        >
                          {file.name}
                        </a>
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3 rounded-b-xl flex-shrink-0">
          <button
            ...
            onClick={onClose}
            disabled={loading}
            className="px-6 py-2.5 border border-gray-300 bg-white text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors text-sm disabled:opacity-50"
          >
            Cancel
          </button>
          <button
