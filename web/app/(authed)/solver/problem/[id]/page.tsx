...
import { getAppealsByProblem } from "@/lib/api/appeals";
...
import type { AppealsByStatus } from "@/types/appeal";
...
function buildHierarchyTree(
  flatList: SolutionAttemptResponse[],
): TreeAttemptNode[] {
    ...
    <Portal>
      ...
      >
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200 overflow-hidden">
          ...
          <div className="flex items-start justify-between p-6 border-b border-gray-100">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                The seeker will review your approach before granting access.
              </p>
            </div>
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors flex-shrink-0 ml-4"
            >
              <svg
                className="w-5 h-5"
                fill="none"
          ...
          {/* Info banner */}
          <div className="mx-6 mt-5 bg-accent/8 border border-accent/20 rounded-lg p-3 flex gap-2.5">
            <svg
            ...
            <p className="text-xs text-accent leading-relaxed">
              Once approved, you'll be able to claim and start working on this
              problem. You won't be locked in until you officially claim it.
            </p>
          </div>
      ...
      >
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-gray-200 p-6">
          ...
          <h2 className="text-lg font-bold text-gray-900 text-center mb-1">
            Build Upon This Solution?
          </h2>
          <p className="text-sm text-gray-500 text-center mb-1">
            You're about to fork the attempt by
          </p>
              ...
              <div className="flex-1 min-w-0 pr-4">
                ...
                <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                  ...
                  <span className="text-[11px] text-gray-500">
                    {
                      node.submissions.filter((s) => s.status === "SUBMITTED")
                        .length
                    }
                    ...
                    <>
                      ...
                      {activeSub.deltaDescription && (
                        <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
                          ...
                          <p className="text-sm text-blue-900 italic leading-relaxed break-words">
                            {activeSub.deltaDescription}
                          </p>
                        </div>
                      )}
                          ...
                          {viewPanel === "previous" && (
                            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                              ...
                              <p className="text-sm text-gray-600 whitespace-pre-wrap break-words leading-relaxed">
                                {predecessorSub.description ||
                                  "No description provided."}
                              </p>
                              {predecessorSub.fileUrls &&
                                predecessorSub.fileUrls.length > 0 && (
    ...
    function walk(nodes: TreeAttemptNode[]) {
      nodes.forEach((node) => {
        if (node.children.length >= 2) {
          ...
          const childRects = childWrappers.map((el) =>
...
export default function SolverProblemDetailPage() {
  ...
  const [myAttempt, setMyAttempt] = useState<SolutionAttemptResponse | null>(
    null,
  );
  const [attempts, setAttempts] = useState<SolutionAttemptResponse[]>([]);
  const [appeals, setAppeals] = useState<AppealsByStatus | null>(null);
  ...
  function HeaderCTA() {
    ...
    if (canClaimFresh) {
      // Solver has submitted an appeal and is waiting for approval
      if (appealSubmitted) {
        return (
          <span className="inline-flex items-center gap-2 px-5 py-2.5 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg font-medium text-sm cursor-default select-none">
            <svg
            ...
            Approach Submitted · Pending Approval
          </span>
        );
      }
      ...
      return (
        <button
          ...
          disabled={claiming}
          className="px-6 py-2.5 bg-secondary hover:bg-accent text-white rounded-lg font-medium transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
        >
          Submit Approach
        </button>
      );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-accent/20 via-background to-accent/10">
      ...
      {showAppealModal && (
        <AppealModal
          problemId={problemId}
          onSuccess={() => setAppealSubmitted(true)}
          onClose={() => setShowAppealModal(false)}
        />
      )}

      {/* Attempt detail modal — rendered via Portal */}
      {modalNode && (
        <AttemptDetailModal
          node={modalNode}
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-8 py-6">
          ...
          <div className="flex items-center gap-3 flex-wrap mt-4">
            <span
              className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold ${STATUS_COLORS[problem.status] || STATUS_COLORS.OPEN}`}
            >
      ...
      <div className="max-w-7xl mx-auto px-8 py-8 space-y-6">
        ...
        {appealSubmitted && !isAlreadyClaimed && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-center gap-3">
            ...
            <p className="text-sm font-medium text-amber-700">
              Your approach has been submitted. You'll be notified once the
              seeker reviews it.
            </p>
          </div>
        )}
              ...
              <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
                <div className="mb-6">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Each node is a solution attempt. Forked nodes branch
                    downward from their parent — click <strong>View</strong> on
                    any card to see details and before/after comparisons.
                  </p>
                </div>
              ...
              <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Submit your approach to get approval, then claim and tackle
                    as many sub-tasks as you want.
                  </p>
                </div>
                {appealSubmitted ? (
                  <span className="inline-flex items-center gap-2 px-5 py-2.5 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg font-medium text-sm cursor-default select-none">
                  ...
                  <button
                    onClick={() => setShowAppealModal(true)}
                    disabled={claiming}
                    className="px-8 py-3 bg-secondary hover:bg-accent text-white font-semibold rounded-lg transition-all shadow-md hover:shadow-lg disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    Submit Approach
                  </button>
                )}
