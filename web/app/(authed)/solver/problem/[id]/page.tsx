...
export default function SolverProblemDetailPage() {
  ...
  const loadData = async () => {
    try {
      ...
      try { setAttempts(await getAllAttempts(problemId)); } catch (err) { console.error("Failed to load history", err); }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-accent/20 via-background to-accent/10 p-8">
        <div className="max-w-4xl mx-auto">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-red-700">{error || "Problem not found"}</div>
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
        ...
        <div className="max-w-7xl mx-auto px-8">
          <div className="flex gap-8">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`py-4 px-2 font-medium text-sm border-b-2 transition-colors ${activeTab === tab.id ? "border-accent text-accent" : "border-transparent text-gray-600 hover:text-gray-900"}`}>
                {tab.label}
              </button>
            ))}
          </div>
